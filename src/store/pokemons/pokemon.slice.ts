...
const formatPokemonSprities = (sprites: Sprites) => {
  ...
  return Object.values(newSprites).filter((sprite) => typeof sprite === "string") as string[];
};
...
const getPokemonDescription = async (id: number): Promise<string> => {
  ...
  const korDescriptions = descriptions?.flavor_text_entries
    ...
    .map(({ flavor_text }: FlavorTextEntry) => flavor_text.replace(/\r|\n|\f/g, " "));
...
export const getPokemonData = createAsyncThunk("pokemon/getPokemonData", async (pokemonName: string, thunkAPI) => {
  try {
    ...
    if (pokemonData) {
      ...
      const nextAndPrevPokemon: NextAndPrevPokemon = await getNextAndPrevPokemon(id);
