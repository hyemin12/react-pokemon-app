...
const DetailPage = () => {
  ...
  const { isLoading, error, pokemonData } = useAppSelector((state) => state.pokemonSlice);
  ...
  if (!isLoading && pokemonData) {
    return (
      <div className='flex items-center flex-col w-full h-full bg-gray-100  dark:bg-gray-800  relative'>
        <header className={`${bg} w-full min-h-[33vh] relative  rounded-bl-[4em] rounded-br-[4em] px-6 py-3`}>
          <div className='flex justify-between items-center'>
            <Link to='/' className='flex items-center gap-1 text-white'>
              <ArrowLeft className='w-6 h-8 text-zinc-200 ' />
              뒤로가기
            </Link>
            <p className='text-zinc-200 font-bold text-md'>#{pokemonData.id.toString().padStart(3, "00")}</p>
          </div>
        ...
        <div className='w-full px-5 py-6 max-w-[550px]'>
          ...
          <section className='flex items-center justify-center gap-4'>{/* 포켓몬 타입 */}</section>
              ...
              <table className='w-full max-w-md m-auto'>
                <tbody>
                  {pokemonData.stats.map(({ name, baseStat }) => (
                    <BaseStat key={name} valueStat={baseStat} nameStat={name} type={pokemonData.types[0]} />
                  ))}
                </tbody>
              </table>
            </div>
        ...
        {pokemonData.prevPokemon && (
          <Link className='absolute top-[40%] -translate-y-1/2 z-9 left-3' to={`/pokemon/${pokemonData.prevPokemon}`}>
            <LessThan className='w-5 h-8 p-1' />
          </Link>
        )}
        {pokemonData.nextPokemon && (
          <Link className='absolute top-[40%] -translate-y-1/2 z-9 right-3' to={`/pokemon/${pokemonData.nextPokemon}`}>
            <GreaterThan className='w-5 h-8 p-1' />
          </Link>
        )}
        {isModalOpen && (
          <DamageModal damages={pokemonData?.damageRelations} open={isModalOpen} setIsModalOpen={setIsModalOpen} />
        )}
      </div>
    );
  }
